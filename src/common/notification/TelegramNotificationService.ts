...
export class TelegramNotificationService {
  ...
  async sendAnalysisResult(
    ...
    let signalMeaning = '';
    if (result.signal === 'BUY') {
      signalMeaning = '💡 <b>매수 신호</b>: 상승 모멘텀이 감지되어 매수 타이밍입니다. 기술적 지표들이 상승 추세를 지지하고 있습니다.';
    } else if (result.signal === 'SELL') {
      signalMeaning = '⚠️ <b>매도 신호</b>: 하락 위험이 감지되어 매도를 고려해야 합니다. 기술적 지표들이 조정 가능성을 시사하고 있습니다.';
    } else {
      signalMeaning = '📊 <b>중립 신호</b>: 현재 명확한 방향성이 없는 상태입니다. 추가적인 신호를 기다리거나 관망하는 것이 좋습니다.';
    }
  ...
  async sendMABreakoutIndividualAlert(
    ...
    let signalMeaning = '';
    if (signalType === 'breakout_up') {
      signalMeaning = '💡 <b>이 신호는 강한 추가상승을 기대할 수 있습니다.</b> 이동평균선 상향돌파는 상승모멘텀이 확보되었음을 의미하며, 추세 전환 가능성이 높습니다.';
    } else {
      signalMeaning = '⚠️ <b>이 신호는 추가하락 위험을 경고합니다.</b> 이동평균선 하향이탈은 하락모멘텀이 강해지고 있음을 의미하며, 손절매를 고려해야 합니다.';
    }
  ...
  async sendRSIThresholdAlert(
    ...
    if (signalType === 'overbought') {
      detailedMeaning = '💡 <b>RSI 과매수 구간 진입으로 단기 조정 가능성이 높습니다.</b> 70 이상은 매수세가 과열된 상태로, 수익 실현 차익거래가 나타날 수 있어 매도를 고려해야 합니다.';
    } else if (signalType === 'oversold') {
      detailedMeaning = '💡 <b>RSI 과매도 구간 진입으로 반등 기회가 나타났습니다.</b> 30 이하는 매도세가 과도한 상태로, 단기 바닥권에서 반등 매수 기회를 제공할 수 있습니다.';
    } else if (signalType === 'bullish_50') {
      detailedMeaning = '💡 <b>RSI 50선 상향돌파로 상승 모멘텀이 확인되었습니다.</b> 중립선 돌파는 매수세 우위를 의미하며, 상승 추세 초기 단계로 판단됩니다.';
    } else {
      detailedMeaning = '⚠️ <b>RSI 50선 하향이탈로 하락 모멘텀이 강화되었습니다.</b> 중립선 이탈은 매도세 우위를 의미하며, 하락 추세 진입 가능성이 높습니다.';
    }
  ...
  async sendMACDSignalAlert(
    ...
    signalLine: number,
    histogram: number,
    signalType: 'golden_cross' | 'dead_cross' | 'bullish_divergence' | 'bearish_divergence',
    confidence: number,
    timestamp: Date = new Date(),
  ): Promise<void> {
    const name = this.getDisplayName(symbol);
    ...
    if (signalType === 'golden_cross') {
      detailedMeaning = '💡 <b>MACD 골든크로스는 강력한 상승신호입니다.</b> 단기 이동평균이 장기 이동평균을 상향돌파하여 상승 모멘텀이 강화되고 있음을 의미합니다. 매수 포지션 진입을 적극 고려해야 합니다.';
    } else if (signalType === 'dead_cross') {
      detailedMeaning = '⚠️ <b>MACD 데드크로스는 강력한 하락신호입니다.</b> 단기 이동평균이 장기 이동평균을 하향이탈하여 하락 모멘텀이 강화되고 있음을 의미합니다. 매도 포지션 진입을 적극 고려해야 합니다.';
    } else if (signalType === 'bullish_divergence') {
      detailedMeaning = '💡 <b>강세 다이버전스는 추세 반전 가능성을 시사합니다.</b> 가격은 하락하지만 MACD는 상승하는 패턴으로, 매도 압력이 약해지고 있어 반등 매수 기회가 될 수 있습니다.';
    } else {
      detailedMeaning = '⚠️ <b>약세 다이버전스는 상승 추세 약화를 경고합니다.</b> 가격은 상승하지만 MACD는 하락하는 패턴으로, 매수 압력이 약해지고 있어 조정 가능성이 높습니다.';
    }
  ...
  async sendBollingerIndividualAlert(
    ...
    lowerBand: number,
    middleBand: number,
    signalType: 'touch_upper' | 'touch_lower' | 'break_upper' | 'break_lower' | 'squeeze',
    confidence: number,
    timestamp: Date = new Date(),
  ): Promise<void> {
    const name = this.getDisplayName(symbol);
    ...
    if (signalType === 'touch_upper') {
      detailedMeaning = '💡 <b>볼린저 상단 터치는 과매수 구간 진입을 의미합니다.</b> 가격이 통계적 상한선에 도달하여 단기 조정 압력이 증가하고 있습니다. 수익 실현 매물 출현 가능성이 높습니다.';
    } else if (signalType === 'touch_lower') {
      detailedMeaning = '💡 <b>볼린저 하단 터치는 과매도 구간 진입을 의미합니다.</b> 가격이 통계적 하한선에 도달하여 단기 반등 가능성이 높아졌습니다. 저점 매수 기회로 활용할 수 있습니다.';
    } else if (signalType === 'break_upper') {
      detailedMeaning = '💡 <b>볼린저 상단 돌파는 강력한 상승 돌파를 의미합니다.</b> 통계적 저항선을 뚫고 상승하여 추가 상승 모멘텀이 확보되었습니다. 추세 가속화 가능성이 높습니다.';
    } else if (signalType === 'break_lower') {
      detailedMeaning = '⚠️ <b>볼린저 하단 이탈은 강력한 하락 신호입니다.</b> 통계적 지지선을 하향 이탈하여 추가 하락 압력이 강화되었습니다. 손절매를 적극 고려해야 합니다.';
    } else {
      detailedMeaning = '💡 <b>볼린저 밴드 스퀴즈는 큰 변동성 움직임을 예고합니다.</b> 밴드 폭이 축소되어 변동성이 낮아진 상태로, 곧 방향성 있는 큰 움직임이 나타날 가능성이 높습니다.';
    }
  ...
  async sendVolumeSpikeAlert(
    ...
    if (signalType === 'volume_surge') {
      detailedMeaning = '💡 <b>거래량 급증은 강력한 관심도 증가를 의미합니다.</b> 평소보다 몇 배 높은 거래량은 큰 손들의 참여나 중요한 뉴스 반영을 시사하며, 가격 움직임이 가속화될 가능성이 높습니다.';
    } else {
      detailedMeaning = '⚠️ <b>거래량 급감은 관심도 하락을 의미합니다.</b> 거래량 부족은 유동성 부족과 가격 안정성 저하를 의미하며, 큰 움직임 시 급격한 변동 가능성이 있습니다.';
    }
  ...
  async sendPriceLevelBreakAlert(
    ...
    const percentText = signalType === 'break_up' ? `+${percent.toFixed(2)}%` : `-${percent.toFixed(2)}%`;
